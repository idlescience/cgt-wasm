...
void bnf(bool &disp, unsigned short int &n, unsigned int &s, vector<double> &excess, double &prec, vector<bool> &unsettled,
           ...
           vector<double> &singleton_bounds, double &epsi_old, bool &nlsu)
{
    ...
    bool u = true;
    bool settled = false;
    subroutine(U, U2, Atight, Atight2, Arref, J, prec, n, t_size, t2_size, rank, disp, Asettled, sr_count, u, s,
               T_coord, T2_coord, unsettled, epsi_old, epsi, unsettled_p, settled, nlsu);
    if (disp)
    {
        cout << endl
